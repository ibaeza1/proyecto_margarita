SELECT do2.name_organization as organization, sum(val_earned) as coverage_uf , sum(val_originalbtgpoints) as original_bettercoins ,sum(val_btgpoints) as showed_bettercoins 
,db.description_btgpointstype
...
left join trusted.dim_contracts dco on dco.pk_dim_contract = fi.pk_dim_contract
WHERE val_factor=100
AND fi.date_habit >= '2022-01-01'
AND fi.date_habit <= '2022-03-31'
AND do2.pk_dim_organization != 312
and dco.is_invalid= false
and dc.pk_dim_currency != 3
and dco.pk_dim_status_contract = 2
